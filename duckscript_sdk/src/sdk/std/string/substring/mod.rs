...
impl Command for CommandImpl {
    ...
    fn run(&self, context: CommandInvocationContext) -> CommandResult {
        if context.arguments.is_empty() {
            ...
            let string_value = context.arguments[0].clone();
            let string_len = string_value.len() as isize;

            let (start, end) = if context.arguments.len() == 1 {
                (0, string_len)
            } else if context.arguments.len() == 2 {
                match parse_number(&context.arguments[1]) {
                    Ok(value) => {
                        if value >= 0 {
                ...
                let start = match parse_number(&context.arguments[1]) {
                    Ok(value) => {
                        if value > (string_len - 1) {
                            return CommandResult::Error(
                ...
                let end = match parse_number(&context.arguments[2]) {
                    Ok(value) => {
                        if value >= start {
                            if value > (string_len - 1) {
                                return CommandResult::Error(
